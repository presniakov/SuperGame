...
class GameSession {
    constructor(socketId, userId, letters) {
        this.score = 0;
        this.history = [];
        this.duration = 3 * 60 * 1000; // 3 mins
        this.isActive = false;
        // Difficulty params
        this.currentSpeed = 10; // units per second
        // Event State
        this.activeSprites = [];
    ...
    startGame(emitSpawn, onGameOver) {
    ...
    scheduleNextEvent(emitSpawn, delay) {
        if (!this.isActive)
            return;
    ...
    processResult(data, emitSpawn) {
        ...
        if (active.length === 0)
            return; // Event already finished or processing
        // Simulate positions? 
        ...
        if (isValidHit) {
            ...
            if (targetIdx !== -1)
                this.activeSprites[targetIdx].active = false;
        }
        else if (result === 'miss') {
    ...
    endGame(onGameOver) {
        ...
        const result = new GameResult_1.default({
